...
class TagsTableTest extends TableTestBase {
    ...
    void testReadWithTagNameNotEqualFilter() throws Exception {
        // Without the read-side post-filter, scan only handles Equal / OR-IN on tag_name and any
        // other predicate (e.g. <>) was silently dropped (fell back to "list all tags"). This
        // case verifies the post-filter properly evaluates such predicates.
        table.createTag("tag-a");
        table.createTag("tag-b");
        table.createTag("tag-c");
    ...
    void testReadWithNonTagNameFieldFilter() throws Exception {
        ...
        // Filter on snapshot_id (index 1) which is not pushed down by scan; only the post-filter
        // path can apply this predicate.
        PredicateBuilder builder = new PredicateBuilder(TagsTable.TABLE_TYPE);
        long maxSnapshotId =
